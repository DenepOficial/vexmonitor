...
class Database:
    def __init__(
        ...
        if not _SAFE_IDENTIFIER.fullmatch(schema):
            raise ValueError(
                "DB_SCHEMA contiene caracteres no válidos."
            )
    ...
    async def migrate_legacy_routes(self) -> None:
        """
        Si existen las tablas de la versión anterior:
          monitor_guilds
          monitored_channels

        copia automáticamente sus rutas a monitor_routes.
